...
class LeagueTest(TestCase):
    def setUp(self):
        self.league_id = 156254
        self.season = 2022
        self.espn_endpoint = "https://fantasy.espn.com/apis/v3/games/FFL/seasons/" + \
            str(self.season) + "/segments/0/leagues/" + str(self.league_id)
        self.players_endpoint = 'https://fantasy.espn.com/apis/v3/games/ffl/seasons/' + \
            str(self.season) + '/players?view=players_wl'
    ...
    def mock_setUp(self, m):
        m.get(self.espn_endpoint + '?view=mTeam&view=mRoster&view=mMatchup&view=mSettings',
              status_code=200, json=self.league_data)
        m.get(self.espn_endpoint + '?view=mDraftDetail',
              status_code=200, json=self.draft_data)
    ...
    def test_get_scoreboard(self, m):
        ...
        self.assertEqual(repr(
            scoreboard[1]), 'Matchup(Team(Watch What  You Saquon), Team(Feel the  Brees))')
        self.assertEqual(scoreboard[0].home_score, 125.5)

        scoreboard = league.scoreboard()
        self.assertEqual(repr(
            scoreboard[-1]), 'Matchup(Team(Jacking Goff  On Sundays), Team(Feel the  Brees))')
    ...
    def test_free_agents(self, m, mock_boxplayer, mock_nfl_schedule, mock_pos_ratings):
        ...
        m.get(self.espn_endpoint + '?view=kona_player_info&scoringPeriodId=16',
              status_code=200, json={'players': [1, 2]})
        league.year = 2019
    ...
    def test_recent_activity(self, m):
        ...
        league.year = 2019
        self.espn_endpoint = "https://fantasy.espn.com/apis/v3/games/ffl/seasons/" + \
            str(2019) + "/segments/0/leagues/" + str(self.league_id)
        ...
        with open('tests/football/unit/data/league_recent_activity_2019.json') as f:
            data = json.loads(f.read())
        m.get(self.espn_endpoint + '/communication/?view=kona_league_communication',
              status_code=200, json=data)
        m.get(self.espn_endpoint + '?view=kona_playercard',
              status_code=200, json=self.player_card_data)

        activity = league.recent_activity()
        self.assertEqual(
            repr(activity[0].actions[0][0]), 'Team(Perscription Mixon)')
        self.assertEqual(len(repr(activity)), 2765)

    @mock.patch.object(League, '_fetch_league')
    def test_cookie_set(self, mock_fetch_league):
        league = League(league_id=1234, year=2019,
                        espn_s2='cookie1', swid='cookie2')
